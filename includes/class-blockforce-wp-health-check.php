...
class BlockForce_WP_Health_Check {
    ...
    public function test_email_delivery() {
        $result = array(
            ...
            'status'      => 'good',
            'badge'       => array(
                'label' => __('Security', $this->text_domain),
                'color' => 'blue',
            ),
            'description' => sprintf(
                '<p>%s</p>',
                __('Email alerts are essential for receiving notifications when your login URL changes.', $this->text_domain)
            ),
            'actions'     => '',
        ...
        if (empty($alert_email) || !is_email($alert_email)) {
            ...
            $result['description'] = sprintf(
                '<p>%s</p>',
                __('No valid email address is configured for security alerts. You will not receive notifications if your login URL changes.', $this->text_domain)
            );
            $result['actions'] = sprintf(
                '<p><a href="%s">%s</a></p>',
                admin_url('options-general.php?page=blockforce-wp&tab=settings'),
                __('Configure Email Settings', $this->text_domain)
            );
        } else {
            $result['description'] .= sprintf(
                '<p>%s <code>%s</code></p>',
                __('Security alerts will be sent to:', $this->text_domain),
    ...
    public function test_rewrite_rules() {
        $result = array(
            ...
            'description' => sprintf(
                '<p>%s</p>',
                __('Rewrite rules allow BlockForce WP to change your login URL dynamically.', $this->text_domain)
            ),
            'actions'     => '',
        ...
        if ($current_slug) {
            ...
            if (!$rule_exists) {
                ...
                $result['description'] = sprintf(
                    '<p>%s</p>',
                    __('The custom login URL rewrite rule was not found. This may cause issues accessing your login page.', $this->text_domain)
                );
                $result['actions'] = sprintf(
                    '<p><a href="%s" class="button button-primary">%s</a></p>',
                    admin_url('options-general.php?page=blockforce-wp&tab=reset'),
                    __('Reset Login URL', $this->text_domain)
                );
            } else {
                $result['description'] .= sprintf(
                    '<p>%s <code>%s</code></p>',
                    __('Custom login URL is active:', $this->text_domain),
    ...
    public function test_configuration() {
        $result = array(
            ...
            'description' => sprintf(
                '<p>%s</p>',
                __('BlockForce WP configuration has been validated.', $this->text_domain)
            ),
            'actions'     => '',
        ...
        if (!empty($issues)) {
            ...
            $result['description'] = '<p>' . __('The following configuration issues were found:', $this->text_domain) . '</p>';
            ...
            $result['actions'] = sprintf(
                '<p><a href="%s">%s</a></p>',
                admin_url('options-general.php?page=blockforce-wp&tab=settings'),
                __('Review Settings', $this->text_domain)
            );
        }
    ...
    public function test_security_status() {
        $result = array(
            ...
            'description' => sprintf(
                '<p>%s</p>',
                __('BlockForce WP is actively monitoring and protecting your WordPress login.', $this->text_domain)
            ),
            'actions'     => '',
        ...
        if (!empty($features)) {
            $result['description'] .= sprintf(
                ...
                __('Active Features:', $this->text_domain),
                implode(', ', $features)
            );
        } else {
            $result['status'] = 'critical';
            ...
            $result['description'] = sprintf(
                '<p>%s</p>',
                __('All protection features are currently disabled. Your site is vulnerable to brute-force attacks.', $this->text_domain)
            );
            $result['actions'] = sprintf(
                '<p><a href="%s" class="button button-primary">%s</a></p>',
                admin_url('options-general.php?page=blockforce-wp&tab=settings'),
                __('Enable Protection', $this->text_domain)
            );
        }
    ...
    /**
     * Add debug information to Site Health Info
     *
    ...
    public function add_debug_information($info) {
        ...
        $info['blockforce-wp'] = array(
            ...
            'fields' => array(
                'version' => array(
                    'label' => __('Plugin Version', $this->text_domain),
                    'value' => '1.1.0',
                ),
                'login_url_status' => array(
                    'label' => __('Login URL Status', $this->text_domain),
                    'value' => $current_slug ? __('Custom URL Active', $this->text_domain) : __('Default URL', $this->text_domain),
                ),
                'current_slug' => array(
                    'label'   => __('Current Login Slug', $this->text_domain),
                    'value'   => $current_slug ? $current_slug : __('None (using wp-login.php)', $this->text_domain),
                    'private' => true,
                ),
                'ip_blocking' => array(
                ...
                'attempt_limit' => array(
                    'label' => __('Attempt Limit', $this->text_domain),
                    'value' => isset($this->settings['attempt_limit']) ? $this->settings['attempt_limit'] : 'N/A',
                ),
                'block_time' => array(
                    'label' => __('Block Time (seconds)', $this->text_domain),
                    'value' => isset($this->settings['block_time']) ? $this->settings['block_time'] : 'N/A',
                ),
                'log_time' => array(
                    'label' => __('Log Time (seconds)', $this->text_domain),
                    'value' => isset($this->settings['log_time']) ? $this->settings['log_time'] : 'N/A',
                ),
                'alert_email' => array(
